...
import { cancelDispatchRequest, getNegotiationById } from '../models/model-index';


const Colors = Color()
function Negotiation({ navigation, appState, route }) {
    const initialFocusRef = React.useRef(null);
    const User = appState.User;
    const [Data, setData] = React.useState(route.params.NewData ? route.params.NewData : null)
    ...
    React.useEffect(() => {
        const unsubscribe = navigation.addListener('focus', async () => {
            if (route.params.id) {
                getNegotiationById(route.params.id)
                    .then(response => {
                        if (response.error) {
                            return false
                        }
    ...
    return (
        <>
            ...
            <SafeAreaView style={styles.container} >
                ...
                {!Data ? <>
                    <Stack style={{ flex: 1, height: 100, justifyContent: "center", alignItems: "item" }} >
                        <ActivityIndicator />
                    </Stack>
                </> :
                    ...
                    <>
                        ...
                        <ScrollView  >
                            ...
                            <VStack style={{
                                ...
                                <BoldText text={`For ${Data.product_objects.name}`} color="#000" />
                            </VStack>
                                ...
                                <VStack>
                                    {Data.payment != null &&
                                        <>
                                            ...
                                            {Data.buyer == User.UUID && Data.dispatch != false ?

                                                <>
                                                    <Center>
                                                        <BoldText size={15} style={{ marginLeft: 10 }} text="Do you need a delivery service?" color="#000" />
                                                    </Center>
                                                    <HStack style={{
                                                        alignItems: "space-between",
                                                        justifyContent: "space-around",
                                                    ...
                                                    {/* <BoldText text="Payment made" color={Colors.dark} /> */}
                                                </>
                                            }
                                        </>
                                    }
